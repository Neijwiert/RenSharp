...
{
    public class RenSharpExampleObserver : RenSharpGameObjObserverClass
    {
        public RenSharpExampleObserver() 
            : base(nameof(RenSharpExampleObserver))
        {
        }
        ...
        public override void TimerExpired(int number, object data)
        {
            if (number == 1)
            {
                Engine.ConsoleOutput($"{nameof(RenSharpExampleObserver)}.{nameof(TimerExpired)}: {nameof(number)}={number}, {nameof(data)}='{data}'.\n");
            }
        }
